...
from textdistance import lcsseq, lcsstr, levenshtein, ratcliff_obershelp
...
if __name__ == "__main__":
    ...
    '''
    parser = argparse.ArgumentParser(description="Aligns sentences with a target text")

    parser.add_argument(
        "sentences_file",
        help="File with sentences to align"
    )
    ...
    parser.add_argument(
        ...
        required=True,
        nargs='+',
        help="Target texts to search and align with the best match"
    )

    parser.add_argument(
        "--output-preliminary-csv",
        default="preliminary_aligns.csv",
        help="Output file for preliminary alignments"
    )

    parser.add_argument(
        "--output-csv",
        default="final_aligns.csv",
        help="Output file for final alignments"
    )

    parser.add_argument(
        "--decimal-csv",
        default=',',
        help="Decimal separator for CSV output"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Minimum similarity to consider a sentence aligned"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Minimum sequence similarity to consider a sentence aligned"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Minimum string similarity to consider a sentence aligned"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Maximum word error rate to consider a sentence aligned"
    )

    parser.add_argument(
        "-pa", "--prefix-postfix-length-acceptance",
        type=int,
        default=None,
        help="Acceptance of prefix and postfix length (in tokens) to consider a sentence aligned"
    )

    parser.add_argument(
        "-ps", "--prefix-postfix-sim-acceptance",
        type=float,
        default=None,
        help="Acceptance of prefix and postfix similarity (percentage) to consider a sentence aligned"
    )
    ...
    with open(args.sentences_file, encoding="utf-8") as txt:
    ...
    for tfp in args.target_texts:
        with open(tfp, encoding="utf-8") as tf:
